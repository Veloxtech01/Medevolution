...
**MedEvolution Inc** — a marketing/catalog website for a US medical
equipment manufacturer. MedEvolution designs and supplies medical equipment
across 14 product lines in 4 divisions:

- **Diagnostics & Imaging** — lab equipment, medical imaging machines,
  nuclear medicine equipment
- **Surgical & Interventional** — operating theatre equipment, robotic
  surgery units, endoscopy/minimally invasive surgery gear, cardiac
  catheterization equipment
- **Critical & Specialty Care** — ICU equipment, radiotherapy/oncology
  machines, maternal/neonatal/pediatric equipment, fertility/IVF equipment,
  dialysis equipment
- **Infrastructure & Turnkey** — modular operating theatres/cleanrooms,
  medical gas systems

Flagship products to feature prominently: **BreastVision TOMO** (3D breast
imaging/mammography system) and **Mobile DR** (mobile digital X-ray unit).

The site also needs to surface: full product documentation (spec sheets,
manuals, service guides) per product, regional support offices (US, Germany,
South Korea, India) plus a distributor network spanning all six continents,
the Nigeria/West Africa partner **GMBE** (handles local installation,
maintenance, and training), and a path for prospective new distributors to
get in touch.
...
and conventions stay consistent across projects. A product/document catalog
of this size will likely need the backend (product data, spec-sheet uploads,
distributor inquiries) sooner than a typical brochure site, but that phase
still hasn't started — see status below.
...
> scaffolded (Vite + React 19 + Tailwind v4 wired via `@tailwindcss/vite`,
> plus router/axios/react-hook-form/react-icons/react-tooltip/
> react-hot-toast/motion installed, oxlint + Vitest configured) with only
> Vite's default placeholder page — no Header/Footer/Home, no admin panel,
> no authentication, no live API calls yet. `/backend` and its stack below
> are the _intended future_ target, carried over from a sibling project —
> do not scaffold or build it until asked.
...
- Installed version is v5 (5.7.0, latest at scaffold time). The
  `data-tooltip-id` API plus a single `<Tooltip />` instance is the intended
  pattern (not the legacy `data-tip` attribute).
...
but the actual cluster and db names for MedEvolution are undecided —
